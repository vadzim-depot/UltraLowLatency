...
namespace Common
{
    ...
    inline auto& GetCurrentTimeStr(std::string *time_str)
    {
        ...
        char szNanosStr[24];
        sprintf(szNanosStr, "%.8s.%09ld", ctime(&time) + 11, std::chrono::duration_cast<std::chrono::nanoseconds>(clock.time_since_epoch()).count() % NANOS_TO_SECS);
        time_str->assign(szNanosStr);
